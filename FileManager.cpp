...
void FileManager::UnLink()
{
	...
	if ((pInode->i_mode & Inode::IFMT) == Inode::IFDIR && !u.SUser())
	{
		...
		this->m_InodeTable->IPut(pInode);
		sys_err("Only SuperUser can delete the root directory");
		return;
	}
	/* 写入清零后的目录项 */
	u.u_IOParam.m_Offset -= (DirectoryEntry::DIRSIZ + 4);
...
void FileManager::ChDir()
{
	...
	if (NULL == pInode)
	{
		sys_err("Path Not Found!");
		return;
	}
	/* 搜索到的文件不是目录文件 */
	if ((pInode->i_mode & Inode::IFMT) != Inode::IFDIR)
	{
		sys_err("The file is not a directory, chdir error!");
		u.u_error = ENOTDIR;
		this->m_InodeTable->IPut(pInode);
		return;
	}
